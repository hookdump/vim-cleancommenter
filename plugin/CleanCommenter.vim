" ============================================================================
" File:        Clean Commenter
" Description: Minimalistic commenter plugin for VIM
" Maintainer:  Ignacio Freiberg <hookdump at gmail dot com>
" Version:     1.0.0
...
if exists("loaded_clean_commenter")
    finish
endif
if v:version < 700
    echoerr "CleanCommenter: this plugin requires vim >= 7. DOWNLOAD IT! You'll thank me later!"
    finish
endif
let loaded_clean_commenter = 1
...
call s:InitVariable("g:CleanCommentWholeLinesInVMode", 0)
...
function s:SetUpForNewFiletype(filetype, forceReset)
    ...
    if has_key(s:delimiterMap, ft)
        let b:CleanCommenterDelims = s:delimiterMap[ft]
        for i in ['left', 'leftAlt', 'right', 'rightAlt']
            if !has_key(b:CleanCommenterDelims, i)
                let b:CleanCommenterDelims[i] = ''
            endif
        endfor
    else
        let b:CleanCommenterDelims = s:CreateDelimMapFromCms()
    endif
...
function s:SwitchToAlternativeDelimiters(printMsgs)
    ...
    if b:CleanCommenterDelims['leftAlt'] == '' && b:CleanCommenterDelims['rightAlt'] == ''
        if a:printMsgs
    ...
    let b:CleanCommenterDelims['left'] = b:CleanCommenterDelims['leftAlt']
    let b:CleanCommenterDelims['right'] = b:CleanCommenterDelims['rightAlt']

    "set the previously current delimiters to be the new alternative ones
    let b:CleanCommenterDelims['leftAlt'] = tempLeft
    let b:CleanCommenterDelims['rightAlt'] = tempRight
...
function s:CommentLinesMinimal(firstLine, lastLine)
    ...
    if !s:HasMultipartDelims()
        throw 'CleanCommenter.Delimiters exception: Minimal comments can only be used for filetypes that have multipart delimiters'
    endif
    ...
    if !g:NERDUsePlaceHolders && s:DoesBlockHaveMultipartDelim(a:firstLine, a:lastLine)
        throw 'CleanCommenter.Settings exception: Place holders are required but disabled.'
    endif
...
function s:CommentLinesSexy(topline, bottomline)
    ...
    if left == -1 || right == -1
        throw 'CleanCommenter.Delimiters exception: cannot perform sexy comments with available delimiters.'
    endif

    "make sure the lines arent already commented sexually
    if !s:CanSexyCommentLines(a:topline, a:bottomline)
        throw 'CleanCommenter.Nesting exception: cannot nest sexy comments'
    endif
...
" Function: CleanComment(mode, type) function {{{2
...
function! CleanComment(mode, type) range
    ...
    if a:type ==? 'Comment' || a:type ==? 'Nested'
        ...
        elseif isVisual && visualmode() == "v" && (g:CleanCommentWholeLinesInVMode==0 || (g:CleanCommentWholeLinesInVMode==2 && s:HasMultipartDelims()))
    ...
    elseif a:type ==? 'Sexy'
        ...
        catch /CleanCommenter.Delimiters/
            call s:CommentLines(forceNested, "none", firstLine, lastLine)
        catch /CleanCommenter.Nesting/
    ...
    elseif a:type ==? 'Yank'
        ...
        execute firstLine .','. lastLine .'call CleanComment("'. a:mode .'", "Comment")'
    endif
    ...
    if isVisual
        ...
        silent! call repeat#set("V" . nlines . "jo" . "\<Plug>CleanCommenter". a:type)
    else
        silent! call repeat#set("\<Plug>CleanCommenter". a:type)
    endif
endfunction
...
function s:UncommentLinesSexy(topline, bottomline)
    ...
    if left == -1 || right == -1
        throw 'CleanCommenter.Delimiters exception: cannot uncomment sexy comments with available delimiters.'
    endif
...
function s:AltMultipart()
    return b:CleanCommenterDelims['rightAlt'] != ''
endfunction
...
function s:Left(...)
    ...
    let delim = has_key(params, 'alt') ? b:CleanCommenterDelims['leftAlt'] : b:CleanCommenterDelims['left'] 
...
function s:RestoreScreenState()
    if !exists("t:NERDComOldTopLine") || !exists("t:NERDComOldPos")
        throw 'CleanCommenter exception: cannot restore screen'
    endif
...
function s:Right(...)
    ...
    let delim = has_key(params, 'alt') ? b:CleanCommenterDelims['rightAlt'] : b:CleanCommenterDelims['right'] 
...
function! s:CreateMaps(modes, target, desc, combo)
    ...
    " 'noremap <silent> <plug>CleanCommenterComment :call CleanComment("n", "Comment")'
    let plug = '<plug>CleanCommenter' . a:target
    let plug_start = 'noremap <silent> ' . plug . ' :call CleanComment("'
    let plug_end = '", "' . a:target . '")<cr>'
    " Build up a menu command like
    " 'menu <silent> comment.Comment<Tab>\\cc <plug>CleanCommenterComment'
...
" call s:CreateMaps('',   ':help CleanCommenterContents<CR>', 'Help', '')

" inoremap <silent> <plug>CleanCommenterInsert <SPACE><BS><ESC>:call CleanComment('i', "insert")<CR>

" switch to/from alternative delimiters (does not use wrapper function)
" nnoremap <plug>CleanCommenterAltDelims :call <SID>SwitchToAlternativeDelimiters(1)<cr>
